...
class MySampler(nn.Module):
    ...
    def forward(self, img):
        code = self.generator(self.transform.HVIT(img))
...
def lut_transform(imgs, luts):
    ...
    Args:
        imgs: 输入图像 (B, 3, H, W)
        luts: 3D LUT (B, 3, D, D, D)
    Returns:
        outs: 变换后的图像 (B, 3, H, W)
    """
    imgs = (imgs - .5) * 2.
    # grid_sample 需要坐标归一化到 [-1, 1]，且通道顺序调整
...
class SimpleUNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Encoder
        self.enc1 = nn.Sequential(nn.Conv2d(in_channels, 32, 3, 1, 1), nn.LeakyReLU(0.2))
        self.enc2 = nn.Sequential(nn.Conv2d(32, 64, 3, 2, 1), nn.LeakyReLU(0.2))  # /2
        self.enc3 = nn.Sequential(nn.Conv2d(64, 64, 3, 2, 1), nn.LeakyReLU(0.2))  # /4

        # Decoder
        self.dec1 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv_dec1 = nn.Sequential(nn.Conv2d(64 + 64, 64, 3, 1, 1), nn.LeakyReLU(0.2))

        self.dec2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv_dec2 = nn.Sequential(nn.Conv2d(64 + 32, 32, 3, 1, 1), nn.LeakyReLU(0.2))

        # Output
        self.final = nn.Conv2d(32, out_channels, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        u1 = self.conv_dec1(u1)

        u2 = self.dec2(u1)
        u2 = torch.cat([u2, x1], dim=1)
        u2 = self.conv_dec2(u2)
...
class SepLUT(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight_generator =Restormer(inp_channels=n_colors, out_channels=n_ranks) #SimpleUNet(in_channels=n_colors, out_channels=n_ranks)
        if n_vertices_3d > 0:
            self.lut3d_generator = LUT3DGenerator(n_colors, n_vertices_3d, n_feats=32, n_ranks=n_ranks)
    ...
    def init_weights(self):
        ...
        self.weight_generator.apply(special_initilization)

        if self.n_vertices_3d > 0:
            self.lut3d_generator.init_weights()

    def forward(self, imgs, origin_imgs=None):
        if origin_imgs is None:
            origin_imgs = imgs

        if self.n_vertices_3d > 0:
            spatial_weights = self.weight_generator.forward(imgs)

            if spatial_weights.shape[-2:] != origin_imgs.shape[-2:]:
                spatial_weights = F.interpolate(spatial_weights, size=origin_imgs.shape[-2:],
                                                mode='bilinear', align_corners=True)

            basis_luts = self.lut3d_generator.basis_luts_bank.weight.t().view(
                self.n_ranks, self.n_colors, *((self.n_vertices_3d,) * self.n_colors)
            )
            b, c, h, w = origin_imgs.shape
            n_ranks = self.n_ranks
            imgs_repeat = origin_imgs.unsqueeze(1).repeat(1, n_ranks, 1, 1, 1).view(b * n_ranks, c, h, w)
            luts_repeat = basis_luts.unsqueeze(0).repeat(b, 1, 1, 1, 1, 1).view(b * n_ranks, c, *basis_luts.shape[2:])
            transformed_imgs = lut_transform(imgs_repeat, luts_repeat)
            transformed_imgs = transformed_imgs.view(b, n_ranks, c, h, w)
            outs = torch.sum(spatial_weights.unsqueeze(2) * transformed_imgs, dim=1)
        else:
            outs = imgs
...
class DepthUncertaintyHeadSingle(nn.Module):
    def __init__(self, dim0=3, out_channels=3, log_range=(-10.0, 10.0)):
        super().__init__()
        self.log_min, self.log_max = log_range

        self.depth_proj = nn.Conv2d(1, dim0, 3, 1, 1, bias=True)

        self.var_conv = nn.Sequential(
            nn.Conv2d(dim0, dim0, 3, 1, 1),
            nn.ELU(),
            nn.Conv2d(dim0, out_channels, 3, 1, 1)  # no activation
        )

    def forward(self, output_hr, depth=None, target_hw=None):
        log_u = self.var_conv(output_hr)

        log_u = torch.clamp(log_u, self.log_min, self.log_max)
        return log_u
...
class RSNet(nn.Module):
    ...
    def __init__(self,
        ...
        if use_depth_uncertainty:
            self.uncert_head = DepthUncertaintyHeadSingle(
                dim0=3, out_channels=uncertainty_out_channels, log_range=log_range
            )
        self.transform = RGB_HVI()

    def forward(self, img, return_uncertainty=True):

        img_unids = F.interpolate(img, size=(self.input_resolution,) * 2, mode='bilinear', align_corners=False)
        unids_hvi=self.transform.HVIT(img_unids)
        param_map, out_feature = self.autoencoder(unids_hvi)
        ...
        log_u = self.uncert_head(self.transform.HVIT(output_hr), depth=None)
        
        log=self.transform.PHVIT(log_u)
